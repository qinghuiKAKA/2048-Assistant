...
(function () {
  ...
  $(".tr3.t_one, .tr3.tac").each(function () {
    ...
    if (href.indexOf("search.php") >= 0) {
      ...
      if (that.find("td:eq(1)").text().match(/豆芽高手/i) || that.find("td:eq(0)").find('a[href*="fid=0"]').length > 0 || that.find("td:eq(0)").text().match(/中字原創|H-GAME|日本騎兵|三級寫真|卡通漫畫|实用漫画|多挂原创|歐美新片|歐美激情|熟女专图|高跟絲襪|街拍精品|网盘二区|网盘三区|高清有碼|AI視界|A I 智能|真实街拍/i)) {
        that.remove();
        return true;
      }
    }
      ...
      /*
      if (that.find("td:eq(2)").text().match(/国产专员|優衣庫|hdgc|日落黄昏|罗马教皇|avp2p|东华帝君|東方秋白|feixue124|YouKu|soav.|myheroman|zgome|魔火帝皇|豆芽高手|1080fuli/i)) {
        that.remove();
        return true;
      }
      */
