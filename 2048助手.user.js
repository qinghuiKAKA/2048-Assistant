...
(function () {
  ...
  // 默认不显示官方预览图
  const checkbox = document.querySelector('input[name="hide_thumb"][value="1"]');
      if (checkbox) {
  ...
  $(".tr3.t_one, .tr3.tac").each(function () {
    ...
    if (href.indexOf("search.php") >= 0) {
      ...
      if (thattd.text().match(/男娘|人妖|伪娘|破壊版|有码|欧美/i) ||
          that.find("td:eq(1)").text().match(/豆芽高手/i) ||
          that.find("td:eq(0)").find('a[href*="fid=0"]').length > 0 ||
          that.find("td:eq(0)").text().match(/中字原創|H-GAME|日本騎兵|三級寫真|卡通漫畫|实用漫画|正片大片|优质图片|唯美清純|亞洲正妹|美足踩踏|绳艺捆绑|多挂原创|歐美新片|歐美激情|熟女专图|高跟絲襪|街拍精品|网盘二区|网盘三区|高清有碼|AI視界|A I 智能|真实街拍|图你所图|小说/i)) {
        that.remove();
        return true;
      }
    }
    ...
    if (url.indexOf("read.php") >= 0 || url.indexOf("state") >= 0 ) {
      GM_xmlhttpRequest({
        ...
        onload: function (result) {
          ...
          //优先在附件元信息区域查找金币字符串（允许空格：100 金币）
          $(doc).find(".attach-meta").each(function() {
            const text = $(this).text();
            const matches = text.match(/\d+\s*金币/g);
            if (matches) matches.forEach(m => goldSet.add(m));
          });
          //如果没找到，继续优先在.f14.cc内查找
          if (goldSet.size === 0) {
            $(doc).find(".f14.cc *").addBack().contents().filter(function() {
              return this.nodeType === 3; // 只处理文本节点
            }).each(function() {
              const text = this.nodeValue;
              //允许数字和"金币"之间有空格,能匹配文本金币格式和附件金币格式
              const matches = text.match(/\d+金币/g);
              if (matches) matches.forEach(m => goldSet.add(m));
            });
          }
          // 如果.f14.cc中还是没找到，全局查找
          if (goldSet.size === 0) {
